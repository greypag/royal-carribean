...
function fadeInit() {
	if (document.getElementById) {
		...
		dots[currentImage].checked = true;
...
function initDots(){
	if (document.getElementById) {
		...
		for (var i =0; i < galleryImages.length; i++){
			galleryDots.innerHTML += '<input type="radio" name="slide-dots" onclick="setCurrentImage('+i+');"/>';
		}
		dots = galleryDots.getElementsByTagName("input");
	}
}
