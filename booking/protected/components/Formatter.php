...
class Formatter extends CFormatter {
    ...
    public function calculatePrice($noOfPeople, $roomModel, $itineraryModel, $promotionCodeObj = null) {
        ...
        if (isset($promotionCodeObj)) {
			...
			$promotionCode_type = $promotionCodeObj->type;                        
        ...
        for ($i = 1; $i <= $noOfPeople; $i++) {
            ...
            if ($promotionCode_type == 'flexible-discount'){
				...
				switch ($i) {
					case 1:
						$promotionCode_ccf_discount_type = $promotionCodeObj->ccf_discount_1_type;
						$promotionCode_ccf_discount = $promotionCodeObj->flexible_discount_ccf_1;
						$promotionCode_nccf_discount_type = $promotionCodeObj->nccf_discount_1_type;
						$promotionCode_nccf_discount = $promotionCodeObj->flexible_discount_nccf_1;
						break;
					case 2:
						$promotionCode_ccf_discount_type = $promotionCodeObj->ccf_discount_2_type;
						$promotionCode_ccf_discount = $promotionCodeObj->flexible_discount_ccf_2;
						$promotionCode_nccf_discount_type = $promotionCodeObj->nccf_discount_2_type;
						$promotionCode_nccf_discount = $promotionCodeObj->flexible_discount_nccf_2;
						break;											
					case 3:
						$promotionCode_ccf_discount_type = $promotionCodeObj->ccf_discount_3_type;
						$promotionCode_ccf_discount = $promotionCodeObj->flexible_discount_ccf_3;
						$promotionCode_nccf_discount_type = $promotionCodeObj->nccf_discount_3_type;
						$promotionCode_nccf_discount = $promotionCodeObj->flexible_discount_nccf_3;
						break;
					case 4:
						$promotionCode_ccf_discount_type = $promotionCodeObj->ccf_discount_4_type;
						$promotionCode_ccf_discount = $promotionCodeObj->flexible_discount_ccf_4;
						$promotionCode_nccf_discount_type = $promotionCodeObj->nccf_discount_4_type;
						$promotionCode_nccf_discount = $promotionCodeObj->flexible_discount_nccf_4;
						break;
				}
            	if ($i <= 2) {
	                $subtotal = $subtotal + $f1;
	                $guestArray[$z]['cruise_fare'] = $f1;
	                $price_withoutNCFF = $f1_withoutNCFF;
	            }
	            else {
	                $subtotal = $subtotal + $f2;
	                $guestArray[$z]['cruise_fare'] = $f2;
	                $price_withoutNCFF = $f2_withoutNCFF;	                
	            }   				  
				if ($promotionCode_ccf_discount_type == '$') {
					$discount = $promotionCode_ccf_discount * -1;
				}
				else {
					$discount = $price_withoutNCFF * $promotionCode_ccf_discount / 100.0 * -1;
				}
				if ($promotionCode_ccf_discount_type == '$') {
					$discount = $discount - $promotionCode_nccf_discount;
				}
				else {
					$discount = $discount - $ncff * $promotionCode_nccf_discount / 100.0;
				}					
	            ...
	            if ($i <= 2) {
                  ...
                  if ($promotionCode[0] != "others")
                  { 
  	                if ($promotionCode[1] == 'all' || $promotionCode[1] == 'onetwo' && $i <= 2 || $promotionCode[1] === 'two' && $i == 2) {
  	                    $discount = Yii::app()->format->promotionOperator($promotionCode, $f1, $promotionCodeObj->figure_per_guest);
  	                    $subtotal = $subtotal + $discount;
  	                    $guestArray[$z]['discount'] = $discount;
  	                }
                  }
                  ...
                  { 
  	                ...
  	                if ($promotionCode[1] == 'all' || $promotionCode[1] == 'threefour') {
  	                    ...
  	                    $discount = Yii::app()->format->promotionOperator($promotionCode, $f2, $promotionCodeObj->figure_per_guest);
  	                    $subtotal = $subtotal + $discount;
  	                    $guestArray[$z]['discount'] = $discount;
  	                }
                  }
