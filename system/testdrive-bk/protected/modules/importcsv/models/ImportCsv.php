...
class ImportCsv extends CFormModel {
    ...
    public function InsertAll($table, $linesArray, $columns, $tableColumns) {
        ...
        for ($k = 0; $k < $linesLength; $k++) {
            ...
            //CVarDumper::dump($tableColumns, 10, true);
            //CVarDumper::dump($linesArray[0][$columns[12]], 10, true);
            //exit();
            for ($i = 0; $i < $columnsLength - 1; $i++) {
                if ($columns[$i] != '') {

                    // 30-3-2015
                    //Sai created this converting method!
                    $checkDate = $linesArray[$k][$columns[$i] - 1];
                    $dt = DateTime::createFromFormat("d/m/Y", $checkDate);
                    if ($dt !== false && !array_sum($dt->getLastErrors())) {



                        //echo $columns[$i];
                        //$linesArray[$k][$columns[$i]] = CDateTimeParser::parse($checkDate, 'dd/mm/yyyy');
                        $linesArray[$k][$columns[$i] - 1] = $dt->getTimestamp();
                    }
                    /*
                      $dt = DateTime::createFromFormat("j/n/Y", $checkDate);
                      if ($dt !== false && !array_sum($dt->getLastErrors())) {

                      //echo $columns[$i];
                      //$linesArray[$k][$columns[$i]] = CDateTimeParser::parse($checkDate, 'dd/mm/yyyy');
                      $linesArray[$k][$columns[$i]] = $dt->getTimestamp();
                      }
                      echo $checkDate;
                      if ($i < $linesLength - 1) {
                      Yii::app()->end();
                    ...
                    /*
                      CVarDumper::dump($linesArray[$k], 10, true);
                      CVarDumper::dump($columns[$i], 10, true);
                      CVarDumper::dump($linesArray[$k][$columns[$i]], 10, true);
                      echo '<br/><br/>';
                     */
                    
                    //CVarDumper::dump($n_in != 0, 10, true);
                    //CVarDumper::dump(htmlspecialchars(stripslashes($linesArray[$k][$columns[$i] - 1]), ENT_QUOTES,Yii::app()->charset), 10, true);
                    //CVarDumper::dump($csvString . "'" . CHtml::encode(stripslashes($linesArray[$k][$columns[$i] - 1])) . "'" , 10, true);
                    //exit();
