...
var s_account="rcciintlroyalchinaprod"
...
s.linkInternalFilters="javascript:,rcclchina.com.cn"          
...
/* Populate eVar32 variable */
        if(!s.eVar32)
		s.eVar32=s.getQueryParam('intcmp');
		s.eVar32=s.getValOnce(s.eVar32,"s_intcmp",0)



/* Populate Country variables 
		if(!s.prop23)
		s.prop23=s.getQueryParam('wuc');
		s.prop23=s.getValOnce(s.prop23,"country",0)*/
...
/*Populate Time Parting prop and eVar */		
	...
	if(temphr)
		s.prop22=s.eVar13=tempweek + " : " + tempday + " : " + temphr;

/*eVars values will be set based on prop values set by client for product search values*/
	...
	if(s.prop21&&!s.eVar40) s.eVar40=s.prop21;
	
/*Populate Site Search Keyword with value from URL identifier 

	if(!s.prop27)
		s.prop27=s.getQueryParam('ip_text');*/

/*Populate Click Source value from the URL identifier 

	if(!s.prop24)
		s.prop24=s.getQueryParam('cS');*/

/* Set Booking Pace - Test to see if Embarkation Date eVar14 is populated*/
   if(s.eVar14)
   {
     var tempToday=new Date()
     var tempCheckindate =new Date(s.eVar14) 

     //Set 1 day in milliseconds
     var tempOneDay=1000*60*60*24

  /*Calculate the difference between the Check-in date and today and set it to Booking Pace eVar*/
   s.eVar29=(Math.ceil((tempCheckindate.getTime()-tempToday.getTime())/(tempOneDay)))
   		if(s.products) 
   		{
 ...
 /* Populate s.hier1 with page name*/
