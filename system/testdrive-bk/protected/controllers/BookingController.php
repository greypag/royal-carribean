    ...
    public function actionServicePromotion() {
        ...
        $promotionCodeObj = PromotionCode::model()->find(
                array(
                    ...
                    'condition' => 'promotion_code ="' . $_POST['Others']['promotion_code'] . '"',
        ));

        if (count($promotionCodeObj) > 0) {

            if ($promotionCodeObj->quota == 0) {
...
// Set cookies
    public function actionStepOneB() {
        ...
        if (isset($_GET['Others']['promotion_code'])) {
            $promotionCodeObj = PromotionCode::model()->find(
                    array(
                        'condition' => 'promotion_code ="' . $_GET['Others']['promotion_code'] . '"',
            ));
    ...
    public function actionStepFour() {
        ...
        try {
            ...
            $promotionCodeObj = PromotionCode::model()->find(
                    array(
                        'condition' => 'promotion_code ="' . $booking_promotion[0] . '"',
            ));
                                ...
                                if ($paymentResponse["status"]) {
									...
									*/
                                    ...
                                    if ($paymentResponse["message"] == "unknown") {
                                        ...
                                        $formError["submit"]["message"] = Yii::t('booking', $paymentResponse["message"]);
                                    }
                                }
                            } else {
    ...
    public function actionDownloadExcel() {
        ...
        foreach ($allBooking as $bookingItem) {
            ...
            if (!empty($allGuest)) {
                ...
                $no_allGuest = count($allGuest);
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $occupancy[$no_allGuest - 1]);
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $bookingItem->itinerary_id);
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $itineraryRoomType[0]['rt']->rt_id .': '. $itineraryRoomType[0]['rt']->rt_name);
